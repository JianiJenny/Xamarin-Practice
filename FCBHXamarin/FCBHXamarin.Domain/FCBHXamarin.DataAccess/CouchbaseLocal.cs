...
using FCBHXamarin.DataAccess.Document_Extensions;
...
namespace FCBHXamarin.DataAccess
{
    ...
    public class CouchbaseLocal<T> : IDataPersistence<T> where T : DomainEntity
    {
        ...
        private ListenerToken DatabaseListenerToken { get; }
        private DatabaseConfiguration DatabaseConfig { get; set; }
        ...
        public CouchbaseLocal(string databaseName = null)
        {
            Limit = 0;
            if (string.IsNullOrEmpty(databaseName))
            {
                DatabaseName = "FCBHXamarin";
            }
            else
            {
            ...
            //If you need to see what is being written to the database locally, uncomment this line and debug.
            //Be aware - this has serious performance implications.
        ...
        private string CreateKey(Guid id)
        {
            ...
            bool caseSensitive = true, bool waitForIndex = false)
        {
            ...
            if (limit == 0)
            {
                query = (ILimit) QueryBuilder.Select(SelectResult.All()).From(DataSource.Database(Database))
        ...
        {
            var expression = QueryBuilder.Select(SelectResult.All()).From(DataSource.Database(Database))
                .Where(Expression.Property("Type").EqualTo(Expression.String(typeof(T).Name))
                    .WithArgs(args));
        ...
        public virtual async Task UpsertAsync(string key, T item)
        {
            ...
            else
            {
                ...
                if (vesselDocumentVersion < documentVersion)
                {
                    ...
                    await Task.Run(() => Database.Save(documentToSave));
                }
        ...
        {
            var query = (ILimit) QueryBuilder.Select(SelectResult.All()).From(DataSource.Database(Database))
        ...
        private async Task<Document> GetCouchbaseDocumentAsync(string key)
        {
        ...
        {
            return await Task.Run(() => BatchInsertSynchronous(list));
        }

        private BatchStatus BatchInsertSynchronous(List<T> list)
        {
            Database.InBatch(() =>
            {
                foreach (var item in list)
                {
                    ...
                    Database.Save(item.ToMutableDocument(key));
                }
            });
        ...
        public async Task<BatchStatus> BatchDeleteAsync(List<Guid> guids, bool forceFailure = false)
        {
            ...
            return await Task.Run(() => BatchDeleteSynchronous(documentList));
        }

        private BatchStatus BatchDeleteSynchronous(List<Document> documentList)
        {
            Database.InBatch(() =>
            {
        ...
        private void CreateIndexes()
        {
            switch (DatabaseName)
            {
                case "administration":
                    ...
                    Database.CreateIndex(
                        "idx_ReferenceNumber",
                        IndexBuilder.ValueIndex(ValueIndexItem.Expression(Expression.Property("ReferenceNumber"))));
                    Database.CreateIndex(
                        "idx_ProjectId",
                    ...
                    Database.CreateIndex(
                        "idx_ParentGroupId",
                        IndexBuilder.ValueIndex(ValueIndexItem.Expression(Expression.Property("ParentGroupId"))));
                    break;
                case "projects":
                    Database.CreateIndex(
                        "idx_Type",
                    ...
                    Database.CreateIndex(
                        "idx_ScriptLineId",
                        IndexBuilder.ValueIndex(ValueIndexItem.Expression(Expression.Property("ScriptLineId"))));
                    break;
                case "scripts":
                    Database.CreateIndex(
                        "idx_Type",
                    ...
                    Database.CreateIndex(
                        "idx_CharacterId",
                        IndexBuilder.ValueIndex(ValueIndexItem.Expression(Expression.Property("CharacterId"))));
                    Database.CreateIndex(
                        "idx_ReaderId",
                        IndexBuilder.ValueIndex(ValueIndexItem.Expression(Expression.Property("ReaderId"))));
                    break;
                case "recordedaudio":
                    Database.CreateIndex(
                        "idx_Type",
